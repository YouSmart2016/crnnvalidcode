...
class CRNN(tf.keras.Model):
    ...
    def __init__(self, num_classes, training):
        ...
        self.conv7 = tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 3), padding="valid", activation=tf.nn.relu)
        self.pool7 = tf.keras.layers.MaxPool2D(pool_size=[2, 2], strides=[2,1])
        # rnn
        ...
        self.birnn1 = tf.keras.layers.Bidirectional(layer=tf.keras.layers.LSTM(256, return_sequences=True), backward_layer=tf.keras.layers.LSTM(256, go_backwards=True, return_sequences=True))
        ...
        self.birnn2 = tf.keras.layers.Bidirectional(layer=tf.keras.layers.LSTM(256, return_sequences=True), backward_layer= tf.keras.layers.LSTM(256, go_backwards=True, return_sequences=True))
    ...
    def call(self, input):
        ...
        x = tf.reshape(x, [-1, x.shape[1]*x.shape[2], x.shape[3]])  # [BATCH, TIME, FILTERS]
        x = self.birnn1(x)
        x = self.birnn2(x)
